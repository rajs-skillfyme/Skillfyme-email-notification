...
-------------------------------------------
Cron job command — runs every minute via Render Cron Job.

Splits learners into 3 chunks and sends at:
  - 3 hours before class → chunk 1
  - 2 hours before class → chunk 2
  - 1 hour before class  → chunk 3 + instructor
"""
...
# Which chunk fires at which offset before class
_CHUNK_OFFSETS = [180, 120, 60]  # minutes before class for each chunk
...
class Command(BaseCommand):
    help = 'Send chunked reminder emails for classes at 3h, 2h, 1h before start'
    ...
    def handle(self, *args, **options):
        ...
        for batch in active_batches:
            ...
            cancelled = Cancellation.objects.filter(
                batch_id=batch.batch_code, cancelled_date=today
            ).first()
            if cancelled:
            ...
            for chunk_index, minutes_before in enumerate(_CHUNK_OFFSETS):
                ...
                logger.info(
                    'Sending chunk %d (%d learners) for batch %s — %d min before class.',
                    chunk_index + 1, len(chunk), batch.batch_code, minutes_before,
                )

                # Send one by one with 1s delay
                ...
                # Instructor email only with last chunk (1 hour before)
